<%@ Page Title="" Language="C#" ValidateRequest="false" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="proEdit.aspx.cs" Inherits="product_Default" EnableEventValidation="false"  %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="mainContent" runat="Server">
    <section>
        ...
        <ul class="ulColumn2">
            ...
            <li><input type="file" name="txtUpload" id="fFile" onchange="return ajaxFileUpload()" style="display:none" />
            ...
            <li>
              ...
              <%--  <asp:DropDownList ID="ddl_produclsid" runat="server"></asp:DropDownList>--%>
               <%-- <select id="ddl_produclsid" runat="server"><option value="0">--请选择--</option></select>--%>
        ...
        <asp:TextBox ID="selectcity" runat="server" style="display:none" OnLoad="selectcity_Load"></asp:TextBox>
                ...
                try {
                    ...
                    $(".ulColumn2").find("input[type='text']").each(function () {
                        ...
                        if (!$(this).val().length && $(this).attr("id") != undefined) {
                            ...
                            if ($(this).attr("id").search("txtCostPrize") >= 0 || $(this).attr("id").search("txtOrderby") >= 0 || $(this).attr("id").search("txtPrize") >= 0 || $(this).attr("id").search("txtCommission") >= 0) {
            ...
            $("#<%=ddl_Category.ClientID %>").change(function () {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        $("#<%=ddl_produclsid.ClientID %>").html(result);
                        //放置id
                        var eurozoneid = $("#<%=ddl_produclsid.ClientID %>").val();
                        $("#<%=selecteurozone.ClientID %>").val(eurozoneid);
                    }
                })
            })
            $("#<%=ddl_produclsid.ClientID %>").change(function () {
                var id = $(this).val();
                $("#<%=selecteurozone.ClientID  %>").val(id);
            })
