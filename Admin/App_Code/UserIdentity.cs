...
public class UserIdentity : IIdentity
{
	public UserIdentity(string ticketData)
	{
        ...
        int __userID = 0;
        int.TryParse(ticketArr[2],out __userID);
    ...
    {
        return string.Format("{0}|{1}|{2}|{3}|{4}|{5}", this._name, this._appID, this._userID, this._weixinCode,this._appSecret,this._shopID);
    }
    ...
    public static bool AuthenticateUser(string username, string password, string role, bool persistLogin)
    {
        ...
        {
            throw new Exception(String.Format("Unable to login user '{0}': " + ex.Message, username), ex);
        }
    }
    /// <summary>
    /// 系统内部的身份验证，自动识别用户角色
